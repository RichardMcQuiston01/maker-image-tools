 ...
 * same rationale as `fakeOAuthProvider.ts`: `mailer.ts`'s `sendMail` is plain
 * REST over HTTP with no SDK to intercept, and there's no way to reach a real
 * provider (or use a real API key) from this sandbox, so `MAIL_API_URL` gets
 * pointed at this instead in tests.
 */
