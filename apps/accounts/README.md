...
signup and login, bearer-token sessions, and a `planTier` field for `@maker/billing` to manage.
Lives in this monorepo as its own workspace app rather than a separate repo — see `ROADMAP.md` §8
for why.
...
`user` is `{ id, email, planTier, createdAt }`. Sessions are bearer tokens (not cookies): the
