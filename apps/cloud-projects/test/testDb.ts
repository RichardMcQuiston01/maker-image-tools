 ...
 * database before running (see apps/cloud-projects/README.md).
 */
export function requireTestPool(): Pool {
  const connectionString = process.env.DATABASE_URL;
  if (!connectionString) {
    throw new Error(
      "DATABASE_URL must be set to a real Postgres connection string to run apps/cloud-projects " +
        "tests (see apps/cloud-projects/README.md).",
    );
  }
...
export async function resetTestDb(pool: Pool): Promise<void> {
  await pool.query("TRUNCATE projects, project_collaborators");
}
