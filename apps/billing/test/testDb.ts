 ...
 * paid Stripe API used elsewhere in this service, a local/CI Postgres
 * instance is free and reproducible, so there's no reason not to exercise
 * the real thing). Point DATABASE_URL at a disposable database before
 * running (see apps/billing/README.md). Stripe itself is mocked - see
 * test/fakeStripe.ts.
 */
export function requireTestPool(): Pool {
  ...
  if (!connectionString) {
    throw new Error(
      "DATABASE_URL must be set to a real Postgres connection string to run apps/billing tests " +
        "(see apps/billing/README.md).",
    );
  }
