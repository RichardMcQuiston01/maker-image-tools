...
It also identifies which planned features are *not* actually about image
...
4. **Only image/vector/G-code processing lives in this repo.** Accounts,
   ...
   generic platform concerns reusable by *other* maker-tool apps — they
...
*Single agent, blocking — everything else depends on this.*
...
*Depends on: 0.2. Fully parallelizable — these are pure, unit-testable
functions with no UI dependency and no shared state, so they're the ideal
first batch of independent agent tasks.*
...
*Depends on: 0.3. Tracks are independent of each other; 2A and 2D depend
only on Stage 1 output existing as an input, not on each other's code.*
...
*Depends on: Stage 1 (raster fill data) + Stage 2 (vector path data).*
...
*Depends on: Stages 1–3. This closes out a legitimate, fully-featured,
100%-client-side v1.*
...
*The "more involved development" tier. Split by where inference runs —
this materially changes cost, latency, and privacy, so treat 5a and 5b as
separate decisions, not one track.*
...
as work *inside* `maker-image-tools` — see §8 for the full breakout. The
...
Building them as separate repos means they can be reused by *any* future
...
*client* packages (`maker-machine-control`, `maker-nesting`) directly as
npm dependencies, and talks to the *service* repos
