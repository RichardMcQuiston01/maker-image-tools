...
| 4D    | House number/address sign generator — same category as 4C (deterministic parametric geometry), but shipped as its own published package, [`@richardmcquiston01/house-number-generator`](https://github.com/RichardMcQuiston01/makertool-house-number-generator), rather than folded into `@maker/generators`; wired into `apps/web`'s `GeneratorsPanel` as a regular npm dependency, matching this repo's own SVG import (`svgToPaths`) for the generated per-glyph/backer cut files | S      |
...
| ------------------------ | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------- |
| `apps/accounts`          | User identity                                                              | Auth (OAuth/email), sessions, entitlements/plan tier                                                                                      | Node/TS + Postgres                  | Stage 6 — **already built**                                                                 |
| `apps/billing`           | Subscriptions & payments                                                   | Stripe integration (checkout, billing portal, webhooks), plan management, usage metering                                                  | Node/TS + Postgres + Stripe         | Stage 6 — **already built**                                                                 |
| `apps/cloud-projects`    | Save/sync/share designs across devices                                     | Project CRUD API, object storage (S3-compatible), shareable links                                                                         | Node/TS + Postgres + object storage | Stage 6 — **already built**                                                                 |
| `apps/community-library` | Shared project/asset marketplace                                           | Browse/search/publish projects, moderation queue, ratings                                                                                 | Node/TS + Postgres + CDN            | Stage 6 — **already built** (object storage stands in for the CDN for now)                  |
| `apps/material-db`       | Crowdsourced material settings (speed/power presets by material × machine) | Submission/review API, versioned presets, search                                                                                          | Node/TS + Postgres                  | Stage 6 — **already built** (Stage 4B's static JSON migrated in via a one-time seed script) |
